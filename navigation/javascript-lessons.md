---
layout: post
title: JavaScript Lessons
permalink: /js/
comments: true
---

In groups of 2-4, we each created a lesson about a JavaScript topic to teach to the rest of the class. Here is a compilation of all of the lessons and homewor

Strings: [Lesson](https://dragon-rg.github.io/portfolio/js/strings) [hw](https://dragon-rg.github.io/portfolio/js/strings/hw)

Mathematical Expressions: [Lesson + hw](https://dragon-rg.github.io/portfolio/js/math)

Booleans: [Lesson + hw](https://dragon-rg.github.io/portfolio/js/booleans)

Arrays: [Lesson + hw](https://dragon-rg.github.io/portfolio/js/arrays)

Iterations: [Lesson](https://dragon-rg.github.io/portfolio/js/iterations) [hw](https://dragon-rg.github.io/portfolio/js/iterations/hw)

Conditionals + Nested Conditionals: [Lesson + hw](https://dragon-rg.github.io/portfolio/js/nested-conditionals)

JSON and Javascript Objects: [Lesson + hw](https://dragon-rg.github.io/portfolio/js/json)

Classes and Methods: [Lesson + hw](https://dragon-rg.github.io/portfolio/js/classes)

Data Abstraction: [Lesson + hw](https://dragon-rg.github.io/portfolio/js/data-abstraction)